...
public class GuiSolarPanel extends GuiContainer
{
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY)
	{
		...
		for(int i = 0; i < SolarPanelTileEntity.INVENTORY_SIZE; ++i)
		{
			...
			{
				int x = 17 + i * 18, y = 59;
				GlStateManager.translate(0.0F, 0.0F, 128.0F);
				this.zLevel = 200.0F;
			...
			if(!nul)
			{
				...
				fontRenderer.drawString(s, 34 + i * 18 - fontRenderer.getStringWidth(s), 68, 0xFFFFFF, true);
			}
			if(inBounds(guiLeft + 17 + i * 18, guiTop + 59, 16, 16, mouseX, mouseY))
			{
				...
				{
					tooltip.clear();
					tooltip.addAll(stack.getTooltip(pinv.player, TooltipFlagSFR.instance));
				}
			}
